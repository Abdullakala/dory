...
type PostgresSchemaListTabProps = {
...
export function PostgresSchemaListTab(props: PostgresSchemaListTabProps) {
    ...
    const loadRows = React.useCallback(async () => {
        ...
        try {
            ...
            if (!isSuccess(res)) {
                throw new Error(res.message || 'Failed to fetch schemas');
            }
            setRows(res.data ?? []);
        } catch (error) {
            console.error('Failed to fetch postgres schemas:', error);
            setRows([]);
        } finally {
            setLoading(false);
        }
