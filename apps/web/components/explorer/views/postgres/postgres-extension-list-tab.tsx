...
type PostgresExtensionListTabProps = {
    database: string;
...
export function PostgresExtensionListTab(props: PostgresExtensionListTabProps) {
    ...
    const loadRows = React.useCallback(async () => {
        ...
        try {
            ...
            if (!isSuccess(res)) {
                throw new Error(res.message || 'Failed to fetch extensions');
            }
            setRows(res.data ?? []);
        } catch (error) {
            console.error('Failed to fetch postgres extensions:', error);
            setRows([]);
        } finally {
            setLoading(false);
        }
    ...
    const visibleRows = React.useMemo(() => {
        ...
        return rows.filter(row => [row.name, row.schema, row.version, row.comment].some(value => (value ?? '').toLowerCase().includes(keyword)));
    }, [rows, searchValue]);
    ...
    const columns = React.useMemo<ColumnDef<DatabaseExtensionMeta>[]>(
        () => [
            ...
            },
            {
                ...
                cell: ({ row }) => (row.original.comment ? <OverflowTooltip text={row.original.comment} className="block max-w-[360px] truncate text-muted-foreground" /> : '-'),
            },
        ],
        [],
    );
