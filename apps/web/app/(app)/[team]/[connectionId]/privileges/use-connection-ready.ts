...
export function usePrivilegesConnectionReady() {
    ...
    useEffect(() => {
        if (!team || !routeConnectionId || isConnectionListLoading || isConnectionLoading || !isCurrentRouteConnection) return;
        if (connectionType === 'clickhouse') return;

        router.replace(`/${team}/${routeConnectionId}/sql-console`);
    }, [connectionType, isConnectionListLoading, isConnectionLoading, isCurrentRouteConnection, routeConnectionId, router, team]);
